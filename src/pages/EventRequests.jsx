 ...
 * Displays all Event Invite requests with filtering, sorting, and action capabilities
 */
function EventRequests() {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const result = await eventRequestService.getAll(filters);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const result = await eventRequestService.getStats();
      if (result.success) {
  ...
  const handleApprove = async (id, data) => {
    const result = await eventRequestService.approve(id, data);
    if (result.success) {
      fetchRequests();
      fetchStats();
    } else {
  ...
  const handleReject = async (id, data) => {
    const result = await eventRequestService.reject(id, data);
    if (result.success) {
      fetchRequests();
      fetchStats();
    } else {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Event Invite Requests</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage and review event invite requests
          </p>
        </div>
      ...
      {!isLoading && requests.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">
            {filters.search || filters.status || filters.eventId
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="text-sm text-gray-700">
                        {request.eventId?.name || request.eventId}
                      </span>
                    </td>
