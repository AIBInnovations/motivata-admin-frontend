...
/**
 * MotivataBlendRequests Page Component
 * Displays all Motivata Blend requests with filtering, sorting, and action capabilities
 */
function MotivataBlendRequests() {
  // State
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const result = await motivataBlendService.getAll(filters);
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const result = await motivataBlendService.getStats();
      if (result.success) {
  ...
  const handleApprove = async (id, data) => {
    const result = await motivataBlendService.approve(id, data);
    if (result.success) {
      fetchRequests();
      fetchStats();
    } else {
  ...
  const handleReject = async (id, data) => {
    const result = await motivataBlendService.reject(id, data);
    if (result.success) {
      fetchRequests();
      fetchStats();
    } else {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Motivata Blend Requests</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage and review Motivata Blend registration requests
          </p>
        </div>
      ...
      {showApproveModal && selectedRequest && (
        <ApproveModal
          ...
          title="Approve Motivata Blend Request"
        />
      )}
      ...
      {showRejectModal && selectedRequest && (
        <RejectModal
          ...
          title="Reject Motivata Blend Request"
        />
      )}
      ...
      {showDetailsModal && selectedRequest && (
        <RequestDetailsModal
          ...
          title="Motivata Blend Request Details"
        />
      )}
    </div>
  );
...
export default MotivataBlendRequests;
