...
import ApprovalModal from '../components/membershipRequests/ApprovalModal';
import RejectionModal from '../components/membershipRequests/RejectionModal';
import RequestDetailsModal from '../components/membershipRequests/RequestDetailsModal';
...
/**
 * MembershipRequests Page Component
 * Displays all membership requests with filtering, sorting, and action capabilities
 */
function MembershipRequests() {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const result = await membershipRequestService.getAll(filters);

      if (result.success) {
        setRequests(result.data.requests || []);
        setPagination(result.data.pagination || pagination);
      } else {
        setError(result.message || 'Failed to fetch membership requests');
        setRequests([]);
      }
    } catch (err) {
  ...
  const handleResendLink = async (request) => {
    ...
    try {
      const result = await membershipRequestService.resendPaymentLink(request._id);
      if (result.success) {
    ...
    <div className="p-6 space-y-6">
      ...
      {!isLoading && requests.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm p-8 text-center">
          ...
          <p className="text-lg font-medium text-gray-900">No membership requests found</p>
          <p className="text-sm text-gray-500 mt-1">
