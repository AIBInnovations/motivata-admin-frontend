 ...
 * Modal for rejecting feature access requests with a reason
 */
function RejectionModal({ request, onClose, onSuccess }) {
  ...
  const handleReject = async () => {
    ...
    try {
      const result = await featureRequestService.reject(request._id, {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title="Reject Feature Request" size="md">
        ...
        <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-lg">
          ...
          <div>
            ...
            <p className="text-sm text-red-700 mt-1">
              This action will reject the feature access request. The user will be notified with the rejection reason you provide.
            </p>
          </div>
        </div>
