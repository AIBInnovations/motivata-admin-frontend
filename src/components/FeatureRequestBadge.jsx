...
/**
 * Badge component that shows pending feature request count
 * Polls the API every 30 seconds for updates
 */
function FeatureRequestBadge() {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      try {
        const result = await featureRequestService.getPendingCount();
        if (result.success) {
          setCount(result.data.count || 0);
          setError(false);
        } else {
          console.warn('Failed to fetch pending feature request count:', result.message);
          setError(true);
        }
      } catch (err) {
        console.error('Failed to fetch pending feature request count:', err);
        setError(true);
      } finally {
        setLoading(false);
      }
...
export default FeatureRequestBadge;
